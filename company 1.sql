...
--
-- Host: mysql
-- Generation Time: Mar 05, 2025 at 08:27 AM
...
CREATE DEFINER=`root`@`%` PROCEDURE `get_users_by_name` (IN `name` VARCHAR(20), IN `lastname` VARCHAR(20))   SELECT * FROM users WHERE user_name = name AND user_lastname = lastname$$

CREATE DEFINER=`root`@`%` PROCEDURE `show_all_users_p` ()   SELECT * FROM show_all_users$$
...
CREATE TABLE `items` (
  ...
  `item_title` varchar(20) COLLATE utf8mb4_general_ci NOT NULL,
...
CREATE TABLE `users` (
  ...
  `user_lastname` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  ...
  `user_verification_key` varchar(36) COLLATE utf8mb4_general_ci DEFAULT NULL,
...
INSERT INTO `users` (`user_pk`, `user_name`, `user_lastname`, `user_email`, `user_username`, `user_password`, `user_created_at`, `user_verification_key`, `user_verified_at`, `user_updated_at`, `user_deleted_at`) VALUES
...
--
ALTER TABLE `users`
  MODIFY `user_pk` bigint UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=58;
